...
render_visible_tile <- function(state, world_x, world_y, player_x, player_y, map_char, theme_colors) {
  ...
  # Check for special room markers
           ...
           !is.null(room <- get_special_room_at(state, world_x, world_y))) {
    cat(color_text(room$char, room$color))
  }
  # Check for random events
  else if (exists("get_event_at") && !is.null(state$events) &&
           !is.null(hit <- get_event_at(state, world_x, world_y))) {
    cat(color_text(hit$event$char, hit$event$color))
  }
  # Map tiles
  else {
